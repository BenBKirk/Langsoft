...
class SettingsPage(QWidget):
    ...
    def __init__(self):
        ...
        self.tabs.addTab(self.tab1,"Dictionary Settings")
        self.tabs.addTab(self.tab2,"Other")
        self.tabs.addTab(self.tab3,"Discourse Highlighter Settings")
    ...
    def first_tab(self):
        ...
        default_button.setMaximumWidth(120)
        default_button.clicked.connect(lambda: self.load_settings(True))
        layout_V.addWidget(default_button)
        layout_V.addWidget(self.dict_table_widget)
        self.tab1.setLayout(layout_V)
    ...
    def load_settings(self,default):
        ...
        if default:
            ...
            tabs = data["tabs"]
        self.dict_table_widget.clearContents()
        self.dict_table_widget.setRowCount(len(tabs)+1)
        self.dict_table_widget.setColumnCount(3)
        self.dict_table_widget.setHorizontalHeaderLabels(["Del","Tab Name", "URL"])
        for i, tab in enumerate(tabs):
            dict_btn = self.make_dict_btn(i)
            self.dict_table_widget.setCellWidget(i,0, dict_btn)
            self.dict_table_widget.setItem(i,1,QTableWidgetItem(tab[0]))
            self.dict_table_widget.setItem(i,2,QTableWidgetItem(tab[1]))
        self.dict_table_widget.resizeColumnsToContents()
        # theme settings ######################################
        if data["dark_theme"]:
        ...
        self.discourse_table_widget.setColumnCount(4)
        self.discourse_table_widget.setRowCount(len(discourse_data.items())+1)
        self.discourse_table_widget.setHorizontalHeaderLabels(["Del","Category","Colour", "List Of Words"])
        index = 0
        for key,value in discourse_data.items():
            color = value["color"]
            dis_btn = self.make_dis_btn(index)
            color_widget = self.make_dis_color_widget(index,color)
            # color_widget.setStyleSheet("background-color : rgb(255,0,255")
            self.discourse_table_widget.setCellWidget(index,0,dis_btn)
            self.discourse_table_widget.setItem(index,1,QTableWidgetItem(key))
            self.discourse_table_widget.setCellWidget(index,2,color_widget)
            self.discourse_table_widget.setItem(index,3,QTableWidgetItem(", ".join(value["list"])))
            index += 1
        self.discourse_table_widget.resizeColumnsToContents()
            
        #grammar highlighting
        # self.text_edit_word_box_1.clear()
        # self.text_edit_word_box_2.clear()
        # time_words = data["grammar_highlighter"]["time_words"]
        # time_words_string = ", ".join(time_words)
        # connecting_words = data["grammar_highlighter"]["connecting_words"]
        # connecting_words_string = ", ".join(connecting_words)
        # self.text_edit_word_box_1.insertPlainText(time_words_string)
        # self.text_edit_word_box_2.insertPlainText(connecting_words_string)
